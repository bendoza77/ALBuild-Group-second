...
export default async function handler(req, res) {
  ...
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD || !process.env.ADMIN_EMAIL) {
    ...
    return res.status(500).json({ error: 'სერვისი კონფიგურირებული არ არის. სცადეთ მოგვიანებით.' })
  }
  ...
  // Respond immediately — user sees success without waiting for SMTP
  res.json({ success: true })

  // Send both emails in the background after response is sent
  Promise.all([
  ...
  ]).catch(err => console.error('Gmail error:', err?.message ?? err))
}
